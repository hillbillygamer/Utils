...
export function* strtok(input, sep = " ", inclusive = false){
	...
	do{
		...
		yield Object.defineProperties({
			number, offset: prev, text,
			last: last || (inclusive
				? prev + text.length >= input.length && text.endsWith(sep)
				: sep === input.slice(prev + text.length)),
			interrupted: last,
			get length(){ return this.text.length; },
			toString()  { return this.text; },
		}, {length: {enumerable: false}, toString: {enumerable: false}});
		if(-1 === next) break;
 ...
 * @property {Boolean} [interrupted] - Is this the final token and not followed by a separator?
 * @property {Boolean} [last] - Is this the final token in the stream?
